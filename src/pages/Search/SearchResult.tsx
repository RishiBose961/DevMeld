...
type SearchResultItem = {
  ...
  fullName?: string;
  username?: string;
  role?: string; // added since you use item.role
};
...
const SearchResult = ({ getSearch }: SearchResultProps) => {
  const results = getSearch?.results ?? []; // ✅ safe default
      ...
      {results.map((item, idx) => {
        if (item.title) {
          ...
          return (
            <div
              key={idx}
              className="p-4 h-56 text-center bg-primary/20 rounded-lg shadow"
            >
              <div className="flex justify-center">
                <img
                  className="size-20 rounded-full"
                  src="https://images.unsplash.com/photo-1757377125320-cf0d54c0c6e8?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxmZWF0dXJlZC1waG90b3MtZmVlZHwyfHx8ZW58MHx8fHx8"
                  alt="avatar"
                />
              </div>
              {item.role === "developer" ? (
